...
def _ensure_brackets(client, account_id_key: str, symbol: str, lifecycle, log_prefix: str = '') -> None:
    ...
    If we are IN_POSITION, we must have broker-recognized STOP + TP0 SELL orders.
    ...
    try:
        ...
        if not lifecycle.stop_order_id and lifecycle.stop is not None and _should_attempt('stop_last_attempt_at'):
            ...
            try:
                ...
                _append_note(lifecycle, f"STOP_PLACE_ERR {str(e)[:160]}")
                # Re-check broker truth in case the broker accepted despite preview oddities
                open_orders = _extract_open_orders_for_symbol(client, account_id_key, symbol)
                _attach_from_broker()

        # 2) TP0 leg
        if not lifecycle.tp0_order_id and lifecycle.tp0 is not None and _should_attempt('tp0_last_attempt_at'):
            ...
            try:
                ...
                if lifecycle.tp0_order_id:
                    ...
                    _log(f"{log_prefix}TP0 placed oid={lifecycle.tp0_order_id} sym={symbol}")
            except Exception as e:
                _append_note(lifecycle, f"TP0_PLACE_ERR {str(e)[:160]}")
                open_orders = _extract_open_orders_for_symbol(client, account_id_key, symbol)
                _attach_from_broker()
    ...
    if lifecycle.stage == "IN_POSITION" and int(lifecycle.filled_qty or 0) > 0:
        try:
            ...
            try:
                ...
                fq = 0
            if fq > 0 and lifecycle.stop_order_id and lifecycle.tp0_order_id:
